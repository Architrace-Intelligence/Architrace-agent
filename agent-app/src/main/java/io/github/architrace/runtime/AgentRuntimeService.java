...
public class AgentRuntimeService {
  ...
  AgentRuntimeService(AgentConfigLoader configLoader, ControlPlaneBootstrapService bootstrapService) {
  ...
  public void start(Path configPath) {
    ...
    var controlPlaneAddress = GrpcAddressParser.parseHostPort(config.control().planeBootstrap().server());
  ...
  private AutoCloseable maybeStartEmbeddedControlPlane(InetSocketAddress address) {
    ...
    ControlPlaneGrpcServer server = new ControlPlaneGrpcServer(address.getPort());
    server.start();
    log.info("Embedded control plane started on {}:{}", host, address.getPort());
    return server;
  }
